...
class FirebaseFirestoreHelper {
  ...
  Future<CategoryModel> addSingleCategory(File image, String name) async {
    DocumentReference refrence = _firebaseFirestore.collection('categories').doc();
    String imageUrl = await FirebaseStorageHelper.instance
        .uploadUserImage(refrence.id, image);

    CategoryModel addCategory =
        CategoryModel(image: imageUrl, id: refrence.id, name: name);
    await refrence.set(addCategory.toJson());
    return addCategory;
  }
  ...
  Future<String> deleteProduct(String categoryId, String productId) async {
    ...
    Future<ProductModel> addSingleProduct(File image , String name , String categoryId , String price , String description) async {
    DocumentReference refrence = _firebaseFirestore.collection('categories').doc(categoryId).collection("products").doc();
    String imageUrl = await FirebaseStorageHelper.instance
        .uploadUserImage(refrence.id, image);

    ProductModel addProduct =
        ProductModel(image: imageUrl, id: refrence.id, name: name , categoryId: categoryId , description: description , isFavourite: false , price: double.parse(price), qty: 1);
    await refrence.set(addProduct.toJson());
    return addProduct;
  }
      Future<List<OrderModel>> getCompletedOrder() async {

    QuerySnapshot<Map<String, dynamic>> completedOrder = await _firebaseFirestore.collection('orders').where("status",isEqualTo: "Completed").get();
    ...
    QuerySnapshot<Map<String, dynamic>> cancelOrder = await _firebaseFirestore .collection('orders').where("status",isEqualTo: "Cancel").get();
    ...
    QuerySnapshot<Map<String, dynamic>> pendingOrder =     await _firebaseFirestore
        .collection('orders').where("status",isEqualTo: "Pending").get();
    ...
    QuerySnapshot<Map<String, dynamic>> deliveryOrder =     await _firebaseFirestore
        .collection('orders').where("status",isEqualTo: "Delivery").get();
